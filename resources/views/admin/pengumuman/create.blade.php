...
<div class="max-w-2xl">
    <form action="{{ route('admin.pengumuman.store') }}" method="POST" class="space-y-5">
        ...
        <div class="glass-card p-6 space-y-5">
            ...
            <div>
                ...
                <textarea name="isi" rows="5"
                    class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-gold/50">{{ old('isi') }}</textarea>
                @error('isi')<p class="text-red-400 text-xs mt-1">{{ $message }}</p>@enderror
            </div>
            <div class="grid grid-cols-2 gap-5">
                ...
                <div>
                    <label class="text-white/70 text-sm mb-2 block">Expired (opsional)</label>
        ...
        <div class="flex gap-3">
            <button type="submit" class="btn-gold">Simpan Pengumuman</button>
